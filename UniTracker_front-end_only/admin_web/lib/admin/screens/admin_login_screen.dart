...
class _AdminLoginScreenState extends State<AdminLoginScreen> {
  ...
  Future<void> _login() async {
    if (_formKey.currentState!.validate()) {
      try {
        // First try local admin credentials
        if (_adminIdController.text == 'admin' &&
            _passwordController.text == 'admin123') {
          // Optionally verify with Supabase database
          try {
            final response = await Supabase.instance.client
                .from('admin_users')
                .select('id, full_name, is_active')
                .eq('id', _adminIdController.text)
            ...
            if (response != null && response['is_active'] == true) {
              ...
              await Supabase.instance.client
                  ...
                  .update({'last_login': DateTime.now().toIso8601String()}).eq(
                      'id', _adminIdController.text);
            }
          } catch (e) {
            // If Supabase fails, still allow local login
            debugPrint('Supabase admin verification failed: $e');
          }
